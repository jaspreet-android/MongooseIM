...
init_per_suite(Config) ->
    case mod_domain_isolation_available() of
        true ->
            instrument_helper:start(
              [{router_stanza_dropped, #{host_type => host_type()}}]),
            escalus:init_per_suite(Config);
        false ->
            {skip, "mod_domain_isolation not enabled in this build"}
    end.
...
modules() ->
    ...
    [{mod_domain_isolation, []},
     {mod_muc_light,
      mod_config(mod_muc_light, #{host => MucHost, backend => Backend})}].
...
routing_one2one_message_to_another_domain_gets_dropped(Config) ->
    ...
    escalus:fresh_story(
      Config, [{alice, 1}, {bob, 1}, {alice_bis, 1}], F).
...
mod_domain_isolation_available() ->
...
receives_service_unavailable(Alice) ->
    ...
    escalus:assert(
      is_error, [<<"cancel">>, <<"service-unavailable">>], Err),
    <<"Filtered by the domain isolation">> = get_error_text(Err).

receives_muc_bad_request(Alice) ->
    Err = escalus:wait_for_stanza(Alice),
    escalus:assert(
      is_error, [<<"modify">>, <<"bad-request">>], Err),
...
verify_alice_has_no_pending_messages(Alice, Bob) ->
    ...
    Stanza = escalus:wait_for_stanza(Alice),
    escalus:assert(
      is_chat_message, [<<"Forces to flush">>], Stanza).
...
assert_stanza_dropped(Sender, Recipient, Stanza) ->
    ...
    instrument_helper:assert_one(
      router_stanza_dropped, #{host_type => host_type()},
      fun(#{count := 1, from_jid := From,
            to_jid := To, stanza := DroppedStanza}) ->
          From =:= SenderJid andalso
          To =:= RecipientJid andalso
          DroppedStanza =:= Stanza
      end).
